...
TrialResult run_trial( mt19937& gen )
{
    ...
    while ( true )
    {
        ...
        if ( rank_key != NN )
        {
            ...
            // rank-deficient key: free KEY and retry (independent of verbose)
            for ( int i = 0; i < NN; i++ )
                delete [] KEY[i];
            delete [] KEY;
            continue;
        ...
        {
            if ( verbose )
                cout << "B does not reach TT rank, run again" << endl;
            // rank-deficient B: free B and KEY, undo the trial count, and retry
            for ( int i = 0; i < TT; i++ )
                delete [] B[i];
            delete [] B;
            for ( int i = 0; i < NN; i++ )
