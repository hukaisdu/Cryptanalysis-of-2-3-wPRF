...
TrialResult run_trial( mt19937& gen )
{
    ...
    if ( rank_key != NN )
    {
        ...
        // rank-deficient key: free KEY and retry
        for ( int i = 0; i < NN; i++ )
            delete [] KEY[i];
        delete [] KEY;
        continue;
    ...
    for ( query = 0; query < ( 1LL << 32 ); query++ )
    {
        ...
        for ( int i = 0; i < NN/4; i++ )
            ss += Input1[i] ^ Input1[i + NN/4] ^ Input1[i + NN/2] ^ Input1[i + 3*NN/4]; 
...
reloop:
    for ( query = 0; query < ( 1LL << 32 ); query++ )
    {
        ...
        int ss = 0;
        for ( int i = 0; i < NN/4; i++ )
            ss += Input2[i] ^ Input2[i + NN/4] ^ Input2[i + NN/2] ^ Input2[i + 3*NN/4]; 
        ...
        if ( rank == RR )
        {
            ...
            for ( int i = 0; i < HalfNN + RR; i++ )
            {
                ...
                if ( res.size() == 0 )
                {
                    ...
                    S2Q += query;
                    // free the in-flight tempb and MatT before retrying
                    delete [] tempb;
                    for ( int k = 0; k < NN; k++ )
                        delete [] MatT[k];
                    delete [] MatT;
    ...
    */
    ...
    for ( auto it : solution2 )
    {
        ...
        if ( Hash_table_for_w1.count( s ) > 0 )
        {
            ...
            for (int i = 0; i < NN; i++ )
                corw[i] = it[i];
            // pin corw to the correct w' when it is present, so the key
            // reconstruction below is not derailed by a filter false positive
            if ( EQ( it, realmid2, NN ) )
                break;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    cout << "W1         " << log2( (double)W1 / TESTNUM ) << endl;
    cout << "W2         " << log2( (double)W2 / TESTNUM ) << endl;
